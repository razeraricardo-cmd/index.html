...
<body>
  ...
  <script>
    // ============================
    // MAPA DE EXAMES CONHECIDOS (Prevent)
    // ============================
    // chave normalizada -> abreviação que será usada na linha final
    const KNOWN_EXAMS = {
      "UREIA": "Ureia",
      "CREATININA": "Cr",
      "SODIO": "Na",
    ...
    // Normaliza string para chave (remove acento e caracteres estranhos)
    function normalizeNameKey(name) {
      let s = (name || "").toUpperCase();
      s = s.normalize("NFD").replace(/[\u0300-\u036f]/g, ""); // tira acentos
      s = s.replace(/[^A-Z ]+/g, " "); // tira números/pontuação
      return s.replace(/\s+/g, " ").trim();
    }

    // Ajusta decimais tipo 2.20 -> 2,20 (mas não mexe em 10.910)
    ...
    function parseExamsFromText(rawText) {
      ...
      // 1) Primeiro: tentar pegar os exames "típicos" da Prevent com regex dedicadas

      function tryMatch(labelKey, displayLabel, regex) {
        const match = text.match(regex);
        if (match && match[1]) {
          let val = normalizeValue(match[1]);
          const key = normalizeNameKey(labelKey);
          if (!usedKeys.has(key)) {
            usedKeys.add(key);
      ...
      // UREIA 148 156 104(*) ...
      tryMatch("UREIA", "Ureia", /\bUREIA\s+([-+]?\d+[.,]?\d*)/i);

      // CREATININA 2.20 2.47 1.84(*) ...
      tryMatch("CREATININA", "Cr", /\bCREATININA\s+([-+]?\d+[.,]?\d*)/i);

      // SÓDIO 136 138 ...
      tryMatch("SODIO", "Na", /\bS[ÓO]DIO\s+([-+]?\d+[.,]?\d*)/i);

      // POTASSIO 4.5 4.5 ...
      tryMatch("POTASSIO", "K", /\bPOT[ÁA]SSIO\s+([-+]?\d+[.,]?\d*)/i);

      // PCR 30.6 ...
      tryMatch("PCR", "PCR", /\bPCR\s+([-+]?\d+[.,]?\d*)/i);

      // TGO 246 ...
      tryMatch("TGO", "TGO", /\bTGO\s+([-+]?\d+[.,]?\d*)/i);

      // TGP 193 ...
      tryMatch("TGP", "TGP", /\bTGP\s+([-+]?\d+[.,]?\d*)/i);

      // BILI TOTAL 0.40 ...
      tryMatch("BILI TOTAL", "BT", /\bBILI TOTAL\s+([-+]?\d+[.,]?\d*)/i);

      // BILI DIRETA 0.20 ...
      tryMatch("BILI DIRETA", "BD", /\bBILI DIRETA\s+([-+]?\d+[.,]?\d*)/i);

      // BILI INDIRETA 0.20 ...
      tryMatch("BILI INDIRETA", "BI", /\bBILI INDIRETA\s+([-+]?\d+[.,]?\d*)/i);

      // 2) Fallback genérico: qualquer exame tipo “NOME 123,4 mg/dL”
      // que NÃO esteja nos usados, e tenha "cara" de exame (tem unidade típica)
      const fallbackRegex =
        /([A-ZÁÂÃÉÊÍÓÔÕÚÇa-záâãéêíóôõúç]{3,40})\s+([-+]?\d+[.,]?\d*)\s*(mg\/dL|g\/dL|mEq\/L|U\/L|mmol\/L|%|ng\/mL|µg\/mL|mL\/min[^ ]*|UI\/L)?/g;

      let m;
      while ((m = fallbackRegex.exec(text)) !== null) {
        let rawName = m[1].trim();
        let value = normalizeValue(m[2]);
        const unit = m[3] || "";
        const key = normalizeNameKey(rawName);

        if (!key) continue;

        // pula se já foi atendido pelos "típicos"
        if (usedKeys.has(key)) continue;

        // pula coisas claramente que NÃO são exames
        if (key === "CRM" || key === "RQE") continue;

        usedKeys.add(key);

        // se estiver no mapa de conhecidos e por algum motivo não entrou antes
        const knownAbbrev =
          KNOWN_EXAMS[key] || KNOWN_EXAMS[rawName.toUpperCase()] || rawName;

        const label = knownAbbrev;
        const valueWithUnit = unit ? `${value} ${unit}` : value;
        parts.push(`${label} ${valueWithUnit}`);
      }
    ...
    function generateLine() {
      ...
      if (!rawText.trim()) {
        alert("Suba um PDF ou cole o texto do laudo da Prevent antes.");
        return;
      }

      const dateStr = formatDateDots(dateValue);
      const exams = parseExamsFromText(rawText);

      if (!exams.length) {
        document.getElementById("outputText").value =
          (dateStr ? dateStr + ": " : "") +
          "[Nenhum exame reconhecido no texto recebido. Veja se o PDF tem texto ou se o formato do laudo é diferente.]";
        return;
      }

      const line =
        (dateStr ? dateStr + ": " : "") + exams.join(" / ");
    ...
    function handleFileInput(event) {
      ...
      if (ext === "pdf") {
        if (!window["pdfjsLib"]) {
          alert(
            "PDF.js não foi carregado corretamente. Verifique a conexão com a internet."
          );
          return;
        }
          ...
          pdfjsLib
            ...
            .promise.then(function (pdf) {
              ...
              return Promise.all(pagePromises).then(function () {
                input.value =
                  allText.trim() ||
                  "[Não foi possível extrair texto do PDF (pode ser só imagem escaneada).]";
              });
            })
            .catch(function (err) {
              console.error(err);
      ...
      alert("Formato não suportado. Use PDF ou TXT, ou cole o texto manualmente.");
    }
    ...
    function copyOutput() {
      ...
      navigator.clipboard
        .writeText(text)
        .then(() => alert("Linha copiada para a área de transferência."))
        .catch(() =>
    ...
    document
      ...
      .addEventListener("click", generateLine);
    document
